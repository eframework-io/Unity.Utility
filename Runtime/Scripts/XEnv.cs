...
namespace EFramework.Unity.Utility
{
    ...
    public partial class XEnv
    {
        ...
        internal static int mainThread = System.Threading.Thread.CurrentThread.ManagedThreadId;
#endif

        /// <summary>
        /// OnInitialize 是编辑器的初始化方法，确保平台类型正确设置。
        /// </summary>
#if UNITY_EDITOR
        ...
        internal static void OnInitialize()
        {
            try
            {
                var envFile = XFile.PathJoin(ProjectPath, ".env");
                if (XFile.HasFile(envFile))
                {
                    ...
                    foreach (var rawLine in lines)
                    {
                        ...
                        XLog.Notice("XEnv.OnInitialize: set environment variable: {0} = {1} from .env file.", key, value);
                    }
                }
            }
...
#if UNITY_EDITOR
            if (Platform == PlatformType.Unknown) { } // 预先获取平台类型。
#endif
        }
    ...
    {
        /// <summary>
        /// Preferences 是环境配置的编辑器，提供环境相关配置项的管理功能。
        /// </summary>
        public class Preferences : XPrefs.IEditor
        {
            ...
            internal static void OnInitialize()
            {
                ...
                try
                {
                    ...
                    {
                        projectName = Path.GetFileNameWithoutExtension(output);
                    }
                }
                catch { }
#endif
            ...
            void XPrefs.IEditor.OnVisualize(string searchContext, XPrefs.IBase context)
            {
                ...
                Enum.TryParse<AppType>(context.GetString(App, AppDefault), out var appType);
                context.Set(App, UnityEditor.EditorGUILayout.EnumPopup("", appType).ToString());

                GUILayout.Label(new GUIContent("Mode"), GUILayout.Width(60));
                Enum.TryParse<ModeType>(context.GetString(Mode, ModeDefault), out var modeType);
                context.Set(Mode, UnityEditor.EditorGUILayout.EnumPopup("", modeType).ToString());
                UnityEditor.EditorGUILayout.EndHorizontal();

                UnityEditor.EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("Solution"), GUILayout.Width(60));
                context.Set(Solution, UnityEditor.EditorGUILayout.TextField("", context.GetString(Solution, SolutionDefault)));

                GUILayout.Label(new GUIContent("Project"), GUILayout.Width(60));
                context.Set(Project, UnityEditor.EditorGUILayout.TextField("", context.GetString(Project, ProjectDefault)));
                UnityEditor.EditorGUILayout.EndHorizontal();

                UnityEditor.EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("Product"), GUILayout.Width(60));
                context.Set(Product, UnityEditor.EditorGUILayout.TextField("", context.GetString(Product, ProductDefault)));

                GUILayout.Label(new GUIContent("Channel"), GUILayout.Width(60));
                context.Set(Channel, UnityEditor.EditorGUILayout.TextField("", context.GetString(Channel, ChannelDefault)));
                UnityEditor.EditorGUILayout.EndHorizontal();

                UnityEditor.EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("Version"), GUILayout.Width(60));
                context.Set(Version, UnityEditor.EditorGUILayout.TextField("", context.GetString(Version, VersionDefault)));

                GUILayout.Label(new GUIContent("Author"), GUILayout.Width(60));
                context.Set(Author, UnityEditor.EditorGUILayout.TextField("", context.GetString(Author, AuthorDefault)));
                UnityEditor.EditorGUILayout.EndHorizontal();

                UnityEditor.EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("Secret"), GUILayout.Width(60));
                context.Set(Secret, UnityEditor.EditorGUILayout.TextField("", context.GetString(Secret, SecretDefault)));

                GUILayout.Label(new GUIContent("Remote"), GUILayout.Width(60));
                context.Set(Remote, UnityEditor.EditorGUILayout.TextField("", context.GetString(Remote, RemoteDefault)));
                UnityEditor.EditorGUILayout.EndHorizontal();
        ...
        public static string Solution
        {
            get
            {
                ...
                {
                    bSolution = true;
                    solution = XPrefs.Asset.GetString(Preferences.Solution, Preferences.SolutionDefault).Eval(Instance);
                }
                return solution;
            }
        }
        ...
        public static string Project
        {
            get
            {
                ...
                {
                    bProject = true;
                    project = XPrefs.Asset.GetString(Preferences.Project, Preferences.ProjectDefault).Eval(Instance);
                }
                return project;
            }
        }
        ...
        public static string Product
        {
            get
            {
                ...
                {
                    bProduct = true;
                    product = XPrefs.Asset.GetString(Preferences.Product, Preferences.ProductDefault).Eval(Instance);
                }
                return product;
            }
        }
        ...
        public static string Channel
        {
            get
            {
                ...
                {
                    bChannel = true;
                    channel = XPrefs.Asset.GetString(Preferences.Channel, Preferences.ChannelDefault).Eval(Instance);
                }
                return channel;
            }
        }
        ...
        public static string Version
        {
            get
            {
                ...
                {
                    bVersion = true;
                    version = XPrefs.Asset.GetString(Preferences.Version, Preferences.VersionDefault).Eval(Instance);
                }
                return version;
            }
        }
        ...
        public static string Author
        {
            get
            {
                ...
                {
                    bAuthor = true;
                    author = XPrefs.Asset.GetString(Preferences.Author, Preferences.AuthorDefault).Eval(Instance);
                }
                return author;
            }
        }
        ...
        public static AppType App
        {
            get
            {
                ...
                {
                    bApp = true;
                    Enum.TryParse(XPrefs.Asset.GetString(Preferences.App, Preferences.AppDefault), out app);
                }
                return app;
            }
        }
        ...
        public static ModeType Mode
        {
            get
            {
                ...
                {
                    bMode = true;
                    Enum.TryParse(XPrefs.Asset.GetString(Preferences.Mode, Preferences.ModeDefault), out mode);
                }
                return mode;
            }
        }
        ...
        public static string Secret
        {
            get
            {
                ...
                {
                    bSecret = true;
                    if (!XPrefs.HasKey(Preferences.Secret)) XLog.Warn("XEnv.Secret: secret is not set, use default value.");
                    secret = XPrefs.Asset.GetString(Preferences.Secret, Preferences.SecretDefault).Eval(Instance);
                }
                return secret;
            }
        }
        ...
        public static string Remote
        {
            get
            {
                ...
                {
                    bRemote = true;
                    remote = XPrefs.Asset.GetString(Preferences.Remote, Preferences.RemoteDefault).Eval(Instance);
                }
                return remote;
            }
        }
    ...
    public partial class XEnv : XString.IEvaluator
    {
        internal static XEnv instance;

        private static readonly object instanceLock = new();

        /// <summary>
        /// Instance 是环境变量解析器的全局单例，用于处理配置中的环境变量引用。
        /// </summary>
        /// </summary>
        public static XEnv Instance
        {
            get
            {
                if (instance == null)
        ...
        /// <summary>
        /// evalPattern 是环境变量引用的正则表达式模式。
        /// </summary>
        internal static readonly Regex evalPattern = new(@"\$\{Environment\.([^}]+?)\}", RegexOptions.Compiled);
        ...
        string XString.IEvaluator.Eval(string input)
        {
            ...
            string replaceFunc(Match match)
            {
                ...
                if (!visited.Add(key))
                {
                    return $"${{Environment.{key}}}(Recursive)";
                }
                ...
                try
                {
                    ...
                    {
                        // 递归处理嵌套的变量
                        return evalPattern.Replace(value, replaceFunc);
                    }

                    // 4. 处理未知变量
                    return $"${{Environment.{key}}}(Unknown)";
                }
                finally
                {
                    // 清理访问标记
            ...
            return evalPattern.Replace(input, replaceFunc);
        }
    }
}