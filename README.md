...
- [XEnv](Documentation~/XEnv.md) 提供了应用程序的环境管理，支持多平台识别、路径管理、命令行参数解析和环境变量求值等功能。
- [XEvent](Documentation~/XEvent.md) 提供了轻量级的事件管理器，支持多重监听、单次及泛型回调和批量通知等功能。
- [XFile](Documentation~/XFile.md) 简化了对文件和目录的基本操作，支持路径归一化、解压缩文件、文件校验等功能。
- [XLog](Documentation~/XLog.md) 提供了一个遵循 RFC5424 标准的日志系统，支持多适配器管理、日志轮转和结构化标签等特性。
...
- [XPrefs](Documentation~/XPrefs.md) 实现了多源化配置的读写，支持可视化编辑、变量求值和命令行参数覆盖等功能，是一个灵活高效的首选项系统。
- [XString](Documentation~/XString.md) 实现了文本处理、数值转换、加密解密和变量求值等功能，是一个高效的字符串工具类。
