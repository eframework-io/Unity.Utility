...
namespace EFramework.Unity.Utility
{
    ...
    public partial class XLog
    {
        ...
        public static void Print(LevelType level, bool force, LogTag tag, object data, params object[] args)
        {
            ...
            if (usingDefault)
            {
                if (level <= LevelType.Error) Handler.Default.LogFormat(LogType.Error, null, "[{0}] {1}", XTime.Format(log.Time, "MM/dd HH:mm:ss.fff"), log.Text(true));
                else Handler.Default.LogFormat(LogType.Log, null, "[{0}] {1}", XTime.Format(log.Time, "MM/dd HH:mm:ss.fff"), log.Text(true));
            }
            else
            {
